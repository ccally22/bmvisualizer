...
#cuda ssachen ersetzen
#poses = torch.cuda.FloatTensor(np.zeros((batch_size,m.num_pose)))
poses = torch.tensor(np.zeros((batch_size, m.num_pose)), dtype=torch.float32, device=device)
#betas = torch.cuda.FloatTensor(betas)
betas = torch.tensor(betas, dtype=torch.float32, device=device)
#trans = torch.cuda.FloatTensor(np.zeros((batch_size,3)))
trans = torch.tensor(np.zeros((batch_size, 3)), dtype=torch.float32, device=device)
