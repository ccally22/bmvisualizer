...
class SUPR(nn.Module):
    def __init__(self,path_model,num_betas=10 , constrained = True):
        ...
        self.register_buffer('J_regressor', torch.tensor(model['J_regressor'], dtype=torch.float32))
        ...
        self.register_buffer('posedirs', torch.tensor(model['posedirs'], dtype=torch.float32))
        # Mean Shape
        #self.register_buffer('v_template', torch.cuda.FloatTensor(model['v_template']))
        self.register_buffer('v_template', torch.tensor(model['v_template'], dtype=torch.float32))
        # Shape corrective blend shapes
        #self.register_buffer('shapedirs', torch.cuda.FloatTensor(np.array(model['shapedirs'][:,:,:num_betas])))
        self.register_buffer('shapedirs', torch.tensor(np.array(model['shapedirs'][:,:,:num_betas]), dtype=torch.float32))
        # Mesh traingles
