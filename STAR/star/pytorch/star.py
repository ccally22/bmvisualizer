...
class STAR(nn.Module):
    def __init__(self,gender='female',num_betas=10):
        ...
        # change from cuda to torch
        # self.register_buffer('J_regressor', torch.cuda.FloatTensor(J_regressor))
        # hier auch star_model?
        self.register_buffer('J_regressor', torch.tensor(star_model['J_regressor'], dtype=torch.float32))


        # Model skinning weights
        # self.register_buffer('weights', torch.cuda.FloatTensor(star_model['weights']))
        self.register_buffer('weights', torch.tensor(star_model['weights'], dtype=torch.float32))
       ...
       # self.register_buffer('v_template', torch.cuda.FloatTensor(star_model['v_template']))
        self.register_buffer('v_template', torch.tensor(star_model['v_template'], dtype=torch.float32))
        ...
        self.register_buffer('shapedirs', torch.tensor(np.array(star_model['shapedirs'][:,:,:num_betas]), dtype=torch.float32))
